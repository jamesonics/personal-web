...
4. The active site is under `website/`. Run site commands from that directory.
5. Do not push or deploy unless the user explicitly asks.
...
Known anti-pattern in this repository:

- `.internship-causal-section { background: #111a2a; color: white; }` creates an abrupt dark presentation-slide block inside a light personal website. This is a design failure unless the user explicitly approves a dark full-width section. Refactor it toward the existing light blue-gray component system when that page is next edited.
...
7. If deploying, verify the live GitHub Pages URL and important assets after the workflow succeeds.
